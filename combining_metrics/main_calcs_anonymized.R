...
##### importing functions/libraries #####
# libraries
library(sp)           # transforming coordinates
library(raster)       # raster calcuations
library(rgdal)        # reading in data, readOGR() and writeOGR()
library(xlsx)         # reading in xlsx tables. Need 64 bit Java on computer to use this package.
library(rgeos)        # buffering places of interest in calculations
library(pracma)       # interpolation in tables
library(prettymapr)   # 
library(maps)         #
library(rootSolve)    # root solving of Weibull and other distributions
library(Hmisc)        # minor tick marks on plots
library(msm)          # doubly truncated normal distribution
library(VGAM)         # folded normal distribution

##### defining working directories #####
# need to be changed based on machine

## main directory used for input/output of files ##
#setwd('C:\\Users\\Jared\\Documents\\Cornell\\Research\\Publications\\DOE Grant\\CombiningRiskFactorCode\\geothermal_pfa')
setwd('C:\\Users\\whealton_c\\Desktop')
...
# set critical angles (in degrees):
# critical angles come from failure envelope
critical_ang1 <- 65.2
critical_ang2 <- 114.8
...
#Utilization
util_thresh5 <- c(5,12,13.5,16,20,25) # values assigned based on expert judgment
...
##### RESERVOIR ######
# reservoir data is a shapefile with reservoirs at different levels
# this was processed into a series of rasters at different depth ranges
# each range was 500 m
