# code to run the thermal cacluation for depth to 80 degC
...
# first, get vector of values from raster
# next, replace -9999 with NA
# last, set raster values to vector with NAs in it
preds <- values(therm_pred)
errs <- values(therm_err)

preds[which(preds %in% -9999)] <- NA
errs[which(errs %in% -9999)] <- NA

values(therm_pred) <- preds
values(therm_err) <- errs
...
# removing rasters to speed up computations
rm(th_3_0_3_NA,th_5_0_5_NA,therm_pred,therm_err,th_pfa_var3,th_pfa_var5
   ,preds,errs,thvecPFvar3,thvecPFvar5)